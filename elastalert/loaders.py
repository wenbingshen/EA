...
class RulesLoader(object):
    ...
    required_locals = frozenset(['type', 'name', 'index'])
    ...
    rules_mapping = {
        ...
        'derivative': ruletypes.DerivativeAggregationRule,
        'freq_metric_agg': ruletypes.FreqMetricAggregationRule,
        'freq_deriv_agg': ruletypes.FreqDerivativeAggregationRule,
        'bucket_script': ruletypes.BucketScriptAggregationRule,
        'bucket_script_deriv': ruletypes.BucketScriptDerivAggregationRule,
    }

    # Used to map names of alerts to their classes
    alerts_mapping = {
        'postEmail': alerts.HTTPPostEmailAlerter,
        ...
        'postSms': alerts.HTTPPostSmsAlerter,
    ...
    def load_options(self, rule, conf, filename, args=None):
        ...
        try:
            elastalert_logger.info("that now ..")
            elastalert_logger.info(rule)
            self.rule_schema.validate(rule)
            elastalert_logger.info("rule_schema..")
            elastalert_logger.info(self.rule_schema)
    ...
    def load_modules(self, rule, args=None):
        ...
        if not args or not args.debug:
            elastalert_logger.info("There is now ===")
            elastalert_logger.info(rule['alert'])
            elastalert_logger.info(rule)
    ...
    def load_alerts(self, rule, alert_field):
        def normalize_config(alert):
            ...
            elastalert_logger.info("this is now ...=====><")
            elastalert_logger.info(alert)
        ...
        try:
            ...
            elastalert_logger.info("that is now =====<")
            elastalert_logger.info(alert_field)
