    ...
    def add_match(self, match):
        ...
        if change:
            ...
            elastalert_logger.debug(
                "Description of the changed records  " + str(dict(list(match.items()) + list(extra.items()))))
...
class NewTermsRule(RuleType):
    ...
    def __init__(self, rule, args=None):
        ...
        if self.rules.get('use_terms_query'):
            ...
            if not self.rules.get('query_key').endswith('.keyword') and not self.rules.get('query_key').endswith(
                '.raw'):
...
class CardinalityRule(RuleType):
    ...
    def check_for_match(self, key, event, gc=True):
        ...
        time_elapsed = lookup_es_key(event, self.ts_field) - self.first_event.get(key,
        ...
        if (len(self.cardinality_cache[key]) > self.rules.get('max_cardinality', float('inf')) or
            (len(self.cardinality_cache[key]) < self.rules.get('min_cardinality',
                                                               float('-inf')) and timeframe_elapsed)):
...
class BaseAggregationRule(RuleType):
    ...
    def add_data_func(self, timestamp, pre, group_by, key_list, index, resDict):
        """ Recursively query out the group query data returned by ES """
        if index in range(len(key_list)):
            key = key_list[index]
            for key_data in pre[key]['buckets']:
                resDict[group_by[key]] = key_data['key']
                index += 1
                if index in range(len(key_list)):
                    pre = key_data
                    self.add_data_func(timestamp, pre, group_by, key_list, index, resDict)
                    index -= 1
                else:
                    self.unwrap_interval_buckets(timestamp, None, resDict, key_data['interval_aggs']['buckets'])
    ...
    def dictToString(self, resDict):
        resStr = ''
        for value in resDict.values():
            resStr = resStr + '_' + value
        return resStr

    def get_common(self, common_file):
        """ Gets the ES information from an common file.

        :param common_file: Path to the file which contains ES information.
        It can be either an absolute file path or one that is relative to the given rule.
        """
        if os.path.isabs(common_file):
            common_file_path = common_file
        else:
            common_file_path = os.path.join(os.path.dirname(self.rules['rule_file']), common_file)
        common_conf = yaml_loader(common_file_path)
        if 'es_host' not in common_conf or 'es_port' not in common_conf or 'alert' not in common_conf:
            raise EAException('Common file must have es_host and es_port and alert fields')
        self.rules['es_host'] = common_conf.get('es_host')
        self.rules['es_port'] = common_conf.get('es_port')
        self.rules['es_username'] = common_conf.get('es_username', {})
        self.rules['es_password'] = common_conf.get('es_password', {})
        self.rules['alert'] = common_conf.get('alert')
...
class MetricAggregationRule(BaseAggregationRule):
    ...
    def __init__(self, *args):
        ...
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])

        if 'max_threshold' not in self.rules and 'min_threshold' not in self.rules:
            raise EAException(
                "MetricAggregationRule must have at least one of max_threshold and min_threshold")
    ...
    def get_match_str(self, match):
        ...
        message = 'Threshold violation, %s:%s %s %s (min: %s max : %s) \n\n' % (
            self.rules['metric_agg_type'],
            self.rules['metric_agg_key'],
            self.rules['except_msg'],
            match_str,
    ...
    def generate_aggregation_query(self):
        return "{" + '"' + self.metric_key + '"' + ": {" + '"' + self.rules['metric_agg_type'] + '"' + ":" + \
               " {" + '''"field":''' + '"' + self.rules['metric_agg_key'] + '"' + "}}}"

    def check_matches(self, timestamp, query_key, aggregation_data, resDict):
        if self.metric_key in aggregation_data.keys():
            if "compound_query_key" in self.rules:
                self.check_matches_recursive(timestamp, query_key, aggregation_data, self.rules['compound_query_key'],
                                             dict(), resDict)

            else:
                metric_val = aggregation_data[self.metric_key]['value']
                resStr = self.metric_key + "_" + self.dictToString(resDict)
                if self.crossed_thresholds(metric_val):
                    match = {self.rules['timestamp_field']: timestamp,
                             self.metric_key: metric_val,
                             'exception_message': self.rules['except_msg']
                             }
                    match.update(resDict)
                    match['resStr'] = resStr
                    if query_key is not None:
                        match[self.rules['query_key']] = query_key
                    self.add_match(match)

    def check_matches_recursive(self, timestamp, query_key, aggregation_data, compound_keys, match_data, resDict):
        ...
        if 'bucket_aggs' in aggregation_data:
            ...
            if self.crossed_thresholds(metric_val):
                ...
                match_data['exception_message'] = self.rules['except_msg']
                match_data.update(resDict)
                match_data['resStr'] = resStr
    ...
    def add_aggregation_data(self, payload):
        for timestamp, payload_data in payload.items():
            resDict = {}
            if 'group_by' in self.rules:
                group_by = self.rules['group_by']
                key_list = []
                for key in group_by:
                    key_list.append(key)
                self.add_data_func(timestamp, payload_data, group_by, key_list, 0, resDict)
            else:
                self.unwrap_interval_buckets(timestamp, None, resDict, payload_data['interval_aggs']['buckets'])

    def unwrap_interval_buckets(self, timestamp, query_key, resDict, interval_buckets):
        for interval_data in interval_buckets:
            # Use bucket key here instead of start_time for more accurate match timestamp
            # elastalert_logger.info('unwrap_interval_func: ' + str(interval_data))
            self.check_matches(ts_to_dt(interval_data['key_as_string']), query_key, interval_data, resDict)

class FreqMetricAggregationRule(BaseAggregationRule):
    """ A rule that matches when there is a low number of events given a timeframe. """
    required_options = frozenset(['metric_agg_key', 'metric_agg_type'])
    allowed_aggregations = frozenset(['min', 'max', 'avg', 'sum', 'cardinality', 'value_count'])

    def __init__(self, *args):
        super(FreqMetricAggregationRule, self).__init__(*args)
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        self.get_ts = new_get_event_ts(self.ts_field)
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])
        if 'freq_threshold' not in self.rules:
            raise EAException(
                "FreqMetricAggregationRule must have freq_threshold")
        if 'freq_threshold' in self.rules and ('freq_max' not in self.rules.get('freq_threshold')
                                               and 'freq_min' not in self.rules.get('freq_threshold')
                                               or 'timeframe' not in self.rules
                                               or 'num_events' not in self.rules.get('freq_threshold')):
            raise EAException(
                "FreqMetricAggregationRule must have timeframe and num_events and at least one of freq_max and freq_min when hava freq_threshold")

        self.metric_key = 'metric_' + self.rules['metric_agg_key'] + '_' + self.rules['metric_agg_type'] + '_freq'
    ...
    def check_matches(self, timestamp, query_key, aggregation_data, resDict):
        # elastalert_logger.info('check_matches_func: ' + str(aggregation_data))
        if self.metric_key in aggregation_data.keys():
            if "compound_query_key" in self.rules:
                self.check_matches_recursive(timestamp, query_key, aggregation_data, self.rules['compound_query_key'],
                                             dict(), resDict)
                ...
                metric_val = aggregation_data[self.metric_key]['value']
                resStr = self.metric_key + "_" + self.dictToString(resDict)
                if self.crossed_thresholds(timestamp, metric_val, resStr):
                    match = {self.rules['timestamp_field']: timestamp,
                             self.metric_key: metric_val,
                             'exception_message': self.rules['except_msg']
                             }
    ...
    def crossed_thresholds(self, timestamp, metric_value, resStr):
        if metric_value is None:
            return False
        freq_threshold = self.rules.get('freq_threshold')
        if 'freq_max' in freq_threshold and metric_value > freq_threshold['freq_max']:
            if self.metric_freq_threshold(freq_threshold, resStr, timestamp):
                return True
        if 'freq_min' in freq_threshold and metric_value < freq_threshold['freq_min']:
            if self.metric_freq_threshold(freq_threshold, resStr, timestamp):
                return True
        return False

    def metric_freq_threshold(self, freq_threshold, resStr, timestamp):
        event = ({self.ts_field: timestamp, resStr: self.metric_key}, 1)
        self.occurrences.setdefault(resStr,
                                    EventWindow(self.rules['timeframe'], getTimestamp=self.get_ts)).append(
            event)
        if self.occurrences[resStr].count() >= freq_threshold['num_events']:
            self.occurrences.pop(resStr)
            return True
        return False
...
class DerivativeAggregationRule(BaseAggregationRule):
    """ Alarm when the value of the derivative of a given field is above or below the maximum threshold. """
    required_options = frozenset(['metric_agg_key', 'metric_agg_type'])
    allowed_aggregations = frozenset(['min', 'max', 'avg', 'sum', 'cardinality', 'value_count'])

    def __init__(self, *args):
        super(DerivativeAggregationRule, self).__init__(*args)
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])
        if 'max_threshold' not in self.rules and 'min_threshold' not in self.rules:
            raise EAException(
                "DerivativeAggregationRule must have at least one of either max_threshold or min_threshold")
        if 'deriv_interval' not in self.rules:
            raise EAException(
                "DerivativeAggregationRule must have deriv_interval. This field must be included to ensure that the time range for querying data is greater than the frequency at which your monitor collector collects data, because the derivative only makes sense on at least two points")

        out_metric_agg_keys = self.rules['metric_agg_key']
        keys_list = out_metric_agg_keys.split('.')
        keys_str = ''
        for stus in keys_list:
            keys_str = keys_str + "_" + stus

        self.metric_key = 'metric' + keys_str + '_' + self.rules['metric_agg_type']
        self.metric_key_deriv = self.metric_key + '_deriv'
        self.unit_time = self.rules.get('unit', '1m')  # default 1 minute

        if not self.rules['metric_agg_type'] in self.allowed_aggregations:
            raise EAException("metric_agg_type must be one of %s" % (str(self.allowed_aggregations)))

        self.rules['aggregation_deriv_element'] = self.generate_aggregation_query()
    ...
    def generate_aggregation_query(self):
        return "{" + '"' + self.metric_key + '"' + ": {" + '"' + self.rules['metric_agg_type'] + '"' + ":" + \
               " {" + '''"field":''' + '"' + self.rules['metric_agg_key'] + '"' + "}}," + \
               '"' + self.metric_key_deriv + '"' + ": {" + '''"derivative": { "buckets_path": ''' + '"' + \
               self.metric_key + '"' + ''', "unit": ''' + '"' + self.unit_time + '"' + "}}}"

    def check_matches(self, timestamp, query_key, aggregation_data, resDict):
        if self.metric_key_deriv in aggregation_data.keys():
            if "compound_query_key" in self.rules:
                self.check_matches_recursive(timestamp, query_key, aggregation_data, self.rules['compound_query_key'],
                                             dict(), resDict)

            else:
                metric_val = aggregation_data[self.metric_key_deriv]['normalized_value']
                resStr = self.metric_key + "_" + self.dictToString(resDict)
                if self.crossed_thresholds(timestamp, metric_val):
                    match = {self.rules['timestamp_field']: timestamp,
                             self.metric_key_deriv: metric_val,
                             'exception_message': self.rules['except_msg']
                             }
                    match.update(resDict)
                    match['resStr'] = resStr
    ...
    def check_matches_recursive(self, timestamp, query_key, aggregation_data, compound_keys, match_data, resDict):
        ...
        if 'bucket_aggs' in aggregation_data:
            ...
            if self.crossed_thresholds(timestamp, metric_val):
                match_data[self.rules['timestamp_field']] = timestamp
                match_data[self.metric_key_deriv] = metric_val
                match_data['exception_message'] = self.rules['except_msg']
                match_data.update(resDict)
                match_data['resStr'] = resStr
    ...
    def crossed_thresholds(self, timestamp, metric_value):
        if metric_value is None:
            return False
        if 'max_threshold' in self.rules and metric_value > self.rules['max_threshold']:
            return True
...
class FreqDerivativeAggregationRule(BaseAggregationRule):
    """ Alarm when the value of the derivative of a given field is above or below the maximum threshold and num_events times in timeframe. """
    required_options = frozenset(['metric_agg_key', 'metric_agg_type'])
    allowed_aggregations = frozenset(['min', 'max', 'avg', 'sum', 'cardinality', 'value_count'])

    def __init__(self, *args):
        super(FreqDerivativeAggregationRule, self).__init__(*args)
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        self.get_ts = new_get_event_ts(self.ts_field)
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])
        if 'freq_threshold' not in self.rules:
            raise EAException("FreqDerivativeAggregationRule must have freq_threshold")
        if 'deriv_interval' not in self.rules:
            raise EAException(
                "FreqDerivativeAggregationRule must have deriv_interval. This field must be included to ensure that the time range for querying data is greater than the frequency at which your monitor collector collects data, because the derivative only makes sense on at least two points")
        if 'freq_threshold' in self.rules and ('freq_max' not in self.rules.get('freq_threshold')
                                               and 'freq_min' not in self.rules.get('freq_threshold')
                                               or 'timeframe' not in self.rules
                                               or 'num_events' not in self.rules.get('freq_threshold')):
            raise EAException(
                "FreqDerivativeAggregationRule must have timeframe and num_events and at least one of freq_max and freq_min when hava freq_threshold")
        ...
        self.metric_key = 'metric' + keys_str + '_' + self.rules['metric_agg_type']
        self.metric_key_deriv = self.metric_key + '_deriv_freq'
        self.unit_time = self.rules.get('unit', '1m')  # default 1 minute
    ...
    def crossed_thresholds(self, timestamp, metric_value, resStr):
        if metric_value is None:
            return False
        # elastalert_logger.info(">=====================freq_threshold==================<")
        freq_threshold = self.rules.get('freq_threshold')
        # elastalert_logger.info("This is freq_threshold:  " + str(freq_threshold))
        if 'freq_max' in freq_threshold and metric_value > freq_threshold['freq_max']:
            if self.metric_freq_threshold(freq_threshold, resStr, timestamp):
                return True
        if 'freq_min' in freq_threshold and metric_value < freq_threshold['freq_min']:
...
class BucketScriptAggregationRule(BaseAggregationRule):
    """ A rule that matches when there is a low number of events given a timeframe. """
    required_options = frozenset(['bucket', 'script'])

    def __init__(self, *args):
        super(BucketScriptAggregationRule, self).__init__(*args)
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])
        ...
        if 'max_threshold' not in self.rules and 'min_threshold' not in self.rules:
            raise EAException(
                "BucketScriptAggregationRule must have at least one of max_threshold and min_threshold")
    ...
    def get_match_str(self, match):
        match_str = json.dumps(match)
        message = 'Threshold violation, %s:%s %s %s (min: %s max : %s) \n\n' % (
            str(self.rules['bucket']),
            str(self.rules['script']),
            self.rules['except_msg'],
            match_str,
            self.rules.get('min_threshold'),
            self.rules.get('max_threshold')
    ...
    def generate_aggregation_query(self):
        bucket = self.rules.get('bucket', {})
        agg_template = ''
        buckets_path = ''
        buckets_key = 'bucket'
        for key in bucket:
            agg_template = agg_template + self.generate_bucket_template(key, bucket[key])
            buckets_path = buckets_path + '"' + key + '":' + '"' + key + '",'
            buckets_key = buckets_key + '_' + key
        self.buckets_key = buckets_key
        return "{" + agg_template + '"' + buckets_key + '": {' + '''"bucket_script": {''' + '''"buckets_path": {''' \
               + buckets_path[:-1] + '''}, "script": ''' + '"' + self.rules['script'] + '''"}}}'''

    def generate_bucket_template(self, key, value):
        return '"' + key + '"' + ": {" + '"' + value['agg_type'] + '"' + ":" + \
                " {" + '''"field":''' + '"' + value['agg_key'] + '"' + "}}, "

    def check_matches(self, timestamp, query_key, aggregation_data, resDict):
        if self.buckets_key in aggregation_data.keys():
            if "compound_query_key" in self.rules:
                self.check_matches_recursive(timestamp, query_key, aggregation_data, self.rules['compound_query_key'],
                                             dict(), resDict)

            else:
                metric_val = aggregation_data[self.buckets_key]['value']
                resStr = self.buckets_key + "_" + self.dictToString(resDict)
                if self.crossed_thresholds(metric_val):
                    match = {self.rules['timestamp_field']: timestamp,
                             self.buckets_key: metric_val,
                             'exception_message': self.rules['except_msg']
                             }
                    match.update(resDict)
                    match['resStr'] = resStr
    ...
    def check_matches_recursive(self, timestamp, query_key, aggregation_data, compound_keys, match_data, resDict):
        ...
        if 'bucket_aggs' in aggregation_data:
            ...
            if self.crossed_thresholds(metric_val):
                match_data[self.rules['timestamp_field']] = timestamp
                match_data[self.buckets_key] = metric_val
                match_data['exception_message'] = self.rules['except_msg']
                match_data.update(resDict)
                match_data['resStr'] = resStr
...
class BucketScriptDerivAggregationRule(BaseAggregationRule):
    """ A rule that matches when there is a low number of events given a timeframe. """
    required_options = frozenset(['bucket', 'script'])

    def __init__(self, *args):
        super(BucketScriptDerivAggregationRule, self).__init__(*args)
        self.ts_field = self.rules.get('timestamp_field', '@timestamp')
        if self.rules.get('common_conf_file'):
            self.get_common(self.rules['common_conf_file'])
        ...
        if 'max_threshold' not in self.rules and 'min_threshold' not in self.rules:
            raise EAException(
                "BucketScriptDerivAggregationRule must have at least one of max_threshold and min_threshold")
        if 'deriv_interval' not in self.rules:
            raise EAException(
                "BucketScriptDerivAggregationRule must have deriv_interval. This field must be included to ensure that the time range for querying data is greater than the frequency at which your monitor collector collects data, because the derivative only makes sense on at least two points")
    ...
    def generate_aggregation_query(self):
        ...
        for key in bucket:
            ...
            buckets_path = buckets_path + '"' + key + '":' + '"' + key + '",'
            buckets_key = buckets_key + '_' + key
        self.buckets_key_deriv = buckets_key + '_deriv'
        return "{" + agg_template + '"' + buckets_key + '": {' + '''"bucket_script": {''' + '''"buckets_path": {''' \
               + buckets_path[:-1] + '''}, "script": ''' + '"' + self.rules['script'] + '''"}}, ''' + '"' \
               + self.buckets_key_deriv + '": {' + '''"derivative": { "buckets_path": ''' \
               + '"' + buckets_key + '",' + '''"unit": "''' + self.unit_time + '"' + "}}}"
    ...
    def check_matches(self, timestamp, query_key, aggregation_data, resDict):
        if self.buckets_key_deriv in aggregation_data.keys():
            if "compound_query_key" in self.rules:
                self.check_matches_recursive(timestamp, query_key, aggregation_data, self.rules['compound_query_key'],
                                             dict(), resDict)

            else:
                metric_val = aggregation_data[self.buckets_key_deriv]['normalized_value']
                resStr = self.buckets_key_deriv + "_" + self.dictToString(resDict)
                if self.crossed_thresholds(metric_val):
                    match = {self.rules['timestamp_field']: timestamp,
                             self.buckets_key_deriv: metric_val,
                             'exception_message': self.rules['except_msg']
                             }
                    match.update(resDict)
                    match['resStr'] = resStr
    ...
    def check_matches_recursive(self, timestamp, query_key, aggregation_data, compound_keys, match_data, resDict):
        ...
        if 'bucket_aggs' in aggregation_data:
            ...
            if self.crossed_thresholds(metric_val):
                match_data[self.rules['timestamp_field']] = timestamp
                match_data[self.buckets_key_deriv] = metric_val
                match_data['exception_message'] = self.rules['except_msg']
                match_data.update(resDict)
                match_data['resStr'] = resStr
