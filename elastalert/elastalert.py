...
class ElastAlerter(object):
    ...
    def parse_args(self, args):
        ...
        parser.add_argument('--rule', dest='rule',
                            help='Run only a specific rule (by filename, must still be in rules folder)')
        parser.add_argument('--silence', dest='silence',
    ...
    def get_aggregation_query(self, query, rule, query_key, terms_size, timestamp_field='@timestamp'):
        ...
        if bucket_interval_period is not None:
            group_template = self.generate_group_template(rule)
            length = 0
            group_template_str = ''
            for lengthed, group_template_stred in group_template.items():
                length = lengthed * 2
                group_template_str = group_template_stred

            interval_aggs_template = '''
                        "interval_aggs": { "date_histogram": { "field": ''' + '"' + str(timestamp_field) + '"' + '''
                        , "interval": ''' + '"' + str(bucket_interval_period) + '"' + \
                                     ''' }, "aggs": ''' + str(metric_agg_element) + "}}"
            show_template = "{ " + group_template_str + interval_aggs_template
            while length > 0:
                show_template = show_template + "}"
                length -= 1
            aggs_element = json.loads(show_template)
            # elastalert_logger.info('=========================here interval_aggs==================')
            if rule.get('bucket_offset_delta'):
                aggs_element['interval_aggs']['date_histogram']['offset'] = '+%ss' % (rule['bucket_offset_delta'])
        else:
            # elastalert_logger.info('=========================what interval_aggs==================')
    ...
    """ Generate the template code for ES group queries """

    def generate_group_template(self, rule):
        group_by = rule.get('group_by', {})
        # elastalert_logger.info('but now... -' + str(group_by))
        template = ''
        length = 0
        for key in group_by:
            length += 1
            template = template + self.group_template(key, group_by[key])

        return {length: template}

    def group_template(self, key, value):
        return '"' + key + '"' + ''': { "terms": { "field": ''' + '"' + value + '"' + ''' }, "aggs": {'''

    def get_deriv_aggregation_query(self, query, rule, query_key, terms_size, timestamp_field='@timestamp'):
        """ Takes a query generated by get_query and outputs a aggregation query """
        query_element = query['query']
        if 'sort' in query_element:
            query_element.pop('sort')
        metric_agg_element = rule['aggregation_deriv_element']
    ...
    def get_hits(self, rule, starttime, endtime, index, scroll=False):
        ...
        try:
            ...
            if len(res.get('_shards', {}).get('failures', [])) > 0:
                try:
                    errs = [e['reason']['reason'] for e in res['_shards']['failures'] if
                            'Failed to parse' in e['reason']['reason']]
                    if len(errs):
    ...
    def get_hits_count(self, rule, starttime, endtime, index):
        ...
        try:
            res = self.thread_data.current_es.count(index=index, doc_type=rule['doc_type'], body=query,
                                                    ignore_unavailable=True)
        except ElasticsearchException as e:
            # Elasticsearch sometimes gives us GIGANTIC error messages
            # (so big that they will fill the entire terminal buffer)
    ...
    def get_hits_aggregation(self, rule, starttime, endtime, index, query_key, term_size=None):
        ...
        try:
            if not rule['five']:
                elastalert_logger.info('no five')
                res = self.thread_data.current_es.deprecated_search(
                    index=index,
    ...
    def get_deriv_aggregation(self, rule, starttime, endtime, index, query_key, term_size=None):
        rule_filter = copy.copy(rule['filter'])
        deriv_interval = float(rule.get('deriv_interval'))
        compare_time = endtime - datetime.timedelta(minutes=deriv_interval)
        diff = starttime - compare_time
        diff_senconds = diff.days * 86400 + diff.seconds
        if diff_senconds > 0:
            starttime = compare_time
        base_query = self.get_query(
            rule_filter,
            starttime,
            endtime,
        ...
        if term_size is None:
            term_size = rule.get('terms_size', 50)
        query = self.get_deriv_aggregation_query(base_query, rule, query_key, term_size, rule['timestamp_field'])
        try:
            if not rule['five']:
                elastalert_logger.info('no five')
                res = self.thread_data.current_es.deprecated_search(
                ...
                elastalert_logger.info('yes five')
                res = self.thread_data.current_es.deprecated_search(index=index, doc_type=rule.get('doc_type'),
                                                                    body=query, size=0, ignore_unavailable=True)
        except ElasticsearchException as e:
            if len(str(e)) > 1024:
    ...
    def run_query(self, rule, start=None, end=None, scroll=False):
        ...
        elif rule.get('aggregation_deriv_element'):
            data = self.get_deriv_aggregation(rule, start, end, index, rule.get('query_key', None))
        else:
        ...
        try:
            if rule.get(
                'scroll_id') and self.thread_data.num_hits < self.thread_data.total_hits and should_scrolling_continue(
                rule):
    ...
    def get_segment_size(self, rule):
        ...
        if not rule.get('use_count_query') and not rule.get('use_terms_query') and not rule.get(
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        if 'freq_threshold' in rule and num_matches > 0:
            num_matches = rule['freq_threshold']['num_events']
        while rule['type'].matches:
            ...
            if 'resStr' in match:
                silence_cache_key = match['resStr']
                del match['resStr']
            query_key_value = self.get_query_key_value(rule, match)
            if query_key_value is not None:
                silence_cache_key += '.' + query_key_value

            if self.is_silenced(silence_cache_key + "._silence") or self.is_silenced(silence_cache_key):
    ...
    def reset_rule_schedule(self, rule):
        ...
        if rule.get('limit_execution') and rule['next_starttime']:
            ...
            elastalert_logger.info(
                'Pausing %s until next run at %s' % (rule['name'], pretty_ts(rule['next_starttime'])))
    ...
    def upload_dashboard(self, db, rule, match):
        ...
        Returns the url to the dashboard. '''
        # Set time range
        start = ts_add(lookup_es_key(match, rule['timestamp_field']),
    ...
    def send_alert(self, matches, rule, alert_time=None, retried=False):
        ...
        for alert in rule['alert']:
            ...
            try:
                ...
                self.handle_error('Error while running alert %s: %s' % (alert.get_info()['type'], e),
                                  {'rule': rule['name']})
    ...
    def find_pending_aggregate_alert(self, rule, aggregation_key_value=None):
        ...
        try:
            if self.writeback_es.is_atleastsixtwo():
                ...
                res = self.writeback_es.deprecated_search(index=self.writeback_index, doc_type='elastalert', body=query,
                                                          size=1)
            if len(res['hits']['hits']) == 0:
                return None
        except (KeyError, ElasticsearchException) as e:
    ...
    def add_aggregated_alert(self, match, rule):
        ...
        if (not rule['current_aggregate_id'].get(aggregation_key_value) or
            ('aggregate_alert_time' in rule and aggregation_key_value in rule['aggregate_alert_time'] and rule[
                'aggregate_alert_time'].get(aggregation_key_value) < ts_to_dt(
                lookup_es_key(match, rule['timestamp_field'])))):
    ...
    def silence(self, silence_cache_key=None):
        ...
        if not silence_cache_key:
            ...
            elastalert_logger.info("in silence func......")
