...
class HTTPPostSmsAlerter(Alerter):
    """ Requested elasticsearch indices are sent by HTTP POST Sms. Encoded with JSON. """

    def __init__(self, rule):
        super(HTTPPostSmsAlerter, self).__init__(rule)
        if self.rule.get('common_conf_file'):
            self.get_common(self.rule['common_conf_file'])

    def alert(self, matches):
        """ Each match will trigger a POST to the specified endpoint(s). """
        for match in matches:
            pre = match if self.post_all_values else {}
            payload = {}
            payload.update(self.post_static_payload)
            for post_key, es_key in list(self.post_payload.items()):
                pre[post_key] = lookup_es_key(match, es_key)
            headers = {
                "Content-Type": "application/json",
                "Accept": "application/json;charset=utf-8"
            }
            SmsData = {}
            SmsData.update(self.post_phone)
            SmsData["Msg"] = json.dumps(pre, ensure_ascii=False, indent=2)
            payload["SmsData"] = SmsData
            ...
            for url in self.post_url:
                try:
                    ...
                    raise EAException("Error posting HTTP Post Sms alert: %s" % e)
            elastalert_logger.info("HTTP Post Sms alert sent.")

    def get_common(self, common_file):
        """ Gets the Sms information from an common file.

        :param common_file: Path to the file which contains postSms information.
        It can be either an absolute file path or one that is relative to the given rule.
        """
        if os.path.isabs(common_file):
            common_file_path = common_file
        else:
            common_file_path = os.path.join(os.path.dirname(self.rule['rule_file']), common_file)
        common_conf = yaml_loader(common_file_path)
        if 'http_post_url' not in common_conf or 'http_post_static_payload' not in common_conf or 'http_post_phone' not in common_conf:
            raise EAException('Common file must have http_post_url and http_post_static_payload and http_post_phone fields')
        if 'http_post_static_payload' in common_conf and ('User' not in common_conf['http_post_static_payload'] or 'Priority' not in common_conf['http_post_static_payload']):
            raise EAException('Common file must hava User and Priority in http_post_static_payload')
        if 'http_post_phone' in common_conf and 'Phone' not in common_conf['http_post_phone']:
            raise EAException('Common file must have Phone in http_post_phone')
        post_url = common_conf['http_post_url']
        if isinstance(post_url, str):
            post_url = [post_url]
        self.post_url = post_url
        self.post_proxy = common_conf.get('http_post_proxy')
        self.post_payload = common_conf.get('http_post_payload', {})
        self.post_static_payload = common_conf.get('http_post_static_payload', {})
        self.post_phone = common_conf.get('http_post_phone', {})
        self.post_all_values = common_conf.get('http_post_all_values', not self.post_payload)
        self.post_http_headers = common_conf.get('http_post_headers', {})
        self.timeout = common_conf.get('http_post_timeout', 10)
...
class HTTPPostEmailAlerter(Alerter):
    """ Requested elasticsearch indices are sent by HTTP POST. Encoded with JSON. """

    def __init__(self, rule):
        super(HTTPPostEmailAlerter, self).__init__(rule)
        if self.rule.get('common_conf_file'):
            self.get_common(self.rule['common_conf_file'])
    ...
    def alert(self, matches):
        """ Each match will trigger a POST to the specified endpoint(s). """
        for match in matches:
            body = match if self.http_email_all_values else {}
            body.update(self.http_email_static_payload)
            for post_key, es_key in list(self.http_email_payload.items()):
                body[post_key] = lookup_es_key(match, es_key)
            headers = {
                "Content-Type": "application/json",
                "Accept": "application/json;charset=utf-8"
            }
            mail = {}
            mail.update(self.http_post_email)
            mail["body"] = json.dumps(body, ensure_ascii=False, indent=2)

            headers.update(self.http_email_headers)
            email_proxies = {'https': self.http_eamil_proxy} if self.http_eamil_proxy else None
            for url in self.post_email_url:
                try:
                    response = requests.post(url + "&mail=" + json.dumps(mail, cls=DateTimeEncoder),
                                             headers=headers, proxies=email_proxies, timeout=self.http_email_timeout)
                    response.raise_for_status()
                except RequestException as e:
                    raise EAException("Error posting Email HTTP Post alert: %s" % e)
            elastalert_logger.info("HTTP Post Email alert sent.")

    def get_common(self, common_file):
        """ Gets the PostEmail information from an common file.

        :param common_file: Path to the file which contains Email information.
        It can be either an absolute file path or one that is relative to the given rule.
        """
        if os.path.isabs(common_file):
            common_file_path = common_file
            ...
            common_file_path = os.path.join(os.path.dirname(self.rule['rule_file']), common_file)
        common_conf = yaml_loader(common_file_path)
        if 'http_post_email_url' not in common_conf or 'http_post_email' not in common_conf:
            raise EAException('Common file must have http_post_email_url and http_post_email fields')
        if 'http_post_email' in common_conf and ('from' not in common_conf['http_post_email'] or 'to' not in common_conf['http_post_email'] or 'subject' not in common_conf['http_post_email']):
            raise EAException('Common file must hava from and to and subject in http_post_email')
        post_email_url = common_conf['http_post_email_url']
        if isinstance(post_email_url, str):
            post_email_url = [post_email_url]
        self.post_email_url = post_email_url
        self.http_post_email = common_conf['http_post_email']
        self.http_eamil_proxy = common_conf.get('http_eamil_proxy')
        self.http_email_payload = common_conf.get('http_email_payload', {})
        self.http_email_static_payload = common_conf.get('http_email_static_payload', {})
        self.http_email_all_values = common_conf.get('http_email_all_values', not self.http_email_payload)
        self.http_email_headers = common_conf.get('http_email_headers', {})
        self.http_email_timeout = common_conf.get('http_email_timeout', 10)

    def get_info(self):
        return {'type': 'http_email_post',
                'http_post_email_webhook_url': self.post_email_url}
